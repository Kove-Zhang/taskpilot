...
#### REQ-IN-004: 基于多源历史数据的自动持续演化与偏好推演
- **需求概述**：系统需具备自动学习用户工作习惯，并动态重写、调优《个人关注方向》的演化能力，使得提炼结果“越用越懂你”。
- **双向数据融合**：核心调度引擎静默拉取两大独立的持久化日志源：
  1. 用户在 UI 前台手动通过剪贴板或拖拽提取的任务记录 (`history.enc`)。
  2. 后台 IMAP 监听轮询自动提取并已成功推送至 Notion (`isSynced: true`) 的高价值邮件任务 (`email_history.enc`)。
- **智能截断与推演策略**：将上述双源数据基于发生时间戳统一降序重排，严格截取最新的 30 条核心记录，剔除敏感冗余后发送给大模型进行逆向推演，精准分析用户的当前项目重心，并自动覆盖更新本地全局配置的 `personalFocus`。
...
#### REQ-UI-001: 沉浸式全局窗口化与 Spotlight 双模态运行机制
- **需求概述**：为了适应不同的办公风格，系统需提供轻量级悬浮与重量级常驻双模式自由切换。
- **Spotlight 聚光灯模式（默认）**：视窗呈现无边框紧凑布局，一旦用户点击视窗外区域或切换应用程序（失去焦点），系统触发 `onBlur` 事件并自动将程序优雅隐匿于托盘，绝不喧宾夺主。
- **全局窗口化模式 (Global Window Mode)**：用户在设置中开启该模式后，应用将瞬间扩展为宽屏看板级布局，并**永久解除失焦即隐藏的限制**，变成一个允许在桌面常驻、可随时对照查看大量待办和数据的普通窗口。
