...
## 17. 非 Notion 优化执行记录：邮件 HTML 安全阅读与 CSP（2026-08-01，本地未提交）

### 17.1 本轮范围

按“先优化 Notion 以外内容”的优先级，本轮先处理 P0 安全项 **SEC-02：邮件原始 HTML 未消毒直接渲染，且 CSP 被关闭**。本轮不调整 Notion 推送、Notion 状态或自动演进业务逻辑；也没有修改当前工作区中已有的 Provider 传输层改动。

### 17.2 已完成改动

| 优化项 | 实施内容 | 关键文件 |
|---|---|---|
| 邮件 HTML 严格消毒 | 新增本地白名单消毒器，仅保留阅读所需的文本、列表、表格和基础排版标签；删除 `script`、`iframe`、`form`、`svg`、`style`、图片及其他主动内容；移除所有事件属性和行内样式。 | `src/lib/emailHtml.ts` |
| 链接安全控制 | 仅保留 `https:`、`http:`、`mailto:` 链接；其他协议（含 `javascript:`、`data:`、`ftp:`）移除 `href`。保留的链接统一追加 `target="_blank"` 与 `rel="noopener noreferrer"`。 | `src/lib/emailHtml.ts` |
| 所有邮件 HTML 路径统一处理 | 最新邮件正文、折叠历史线程、白底原貌阅读和深色阅读均改用同一消毒器；不再将保存的原始 HTML 直接交给 `dangerouslySetInnerHTML`。 | `src/EmailTasksPanel.tsx` |
| 外部图片/跟踪像素防护 | 邮件中的 `img` / `picture` / `source` 会被移除，因此打开邮件历史不会自动加载外部图片或触发追踪像素。阅读提示更新为“安全阅读”。 | `src/lib/emailHtml.ts`、`src/EmailTasksPanel.tsx` |
| 恢复 CSP | Tauri CSP 从 `null` 收紧为仅允许应用自身脚本、样式、资源与受控 data/asset 图片；禁止 object、frame、form 和外部 connect。 | `src-tauri/tauri.conf.json` |
| 安全回归测试 | 新增合法内容保留、危险标签/事件/外链图片清除、危险 URL 拦截和 `mailto:` 链接保留测试。 | `src/lib/emailHtml.test.ts` |

### 17.3 验证结果

| 检查项 | 结果 |
|---|---:|
| `npm test` | **14 个测试文件、39 个用例全部通过** |
| `npx tsc --noEmit` | 通过 |
| `npm run lint` | 通过 |
| `npm run build` | 通过 |
| `git diff --check` | 通过 |
| `tauri.conf.json` JSON 解析 | 通过 |

构建仍保留既有的非阻断大 chunk 提示（PDF worker、解析模块）；本轮未增加新的构建错误。

### 17.4 风险降低与剩余限制

- 本轮已降低“打开不可信邮件 HTML 时执行发件人脚本、事件属性、iframe、表单、SVG 或加载跟踪图片”的风险。
- 消毒器刻意以“阅读安全优先”为原则，因此会移除原邮件的自定义颜色、内联样式和图片；文字、链接、列表与表格仍可阅读。
- CSP 的最终桌面兼容性仍应在打包后的 Tauri 应用中人工验证：打开邮件历史、内嵌本地图片预览、模型请求和自定义 Provider 请求均应正常。当前前端构建验证已通过，但不替代真实 WebView 联调。
- 本轮没有处理 SEC-03（操作系统凭据库不可用时的密钥回退、历史数据统一加密）和真实 Tauri Driver E2E；这些仍是后续非 Notion 高优先级事项。

### 17.5 后续非 Notion 优化优先级

1. **SEC-03**：移除可预测的密钥回退，建立统一的加密历史存储与迁移/轮换策略；该项会触及当前已有 Rust 改动，应待 Provider 改动核查后再进行。
2. **桌面 E2E**：在隔离环境配置 `tauri-driver`，覆盖邮件安全阅读、设置未保存退出、弹层 Escape/焦点恢复和失败恢复路径。
3. **可访问性与交互一致性**：统一 Dialog、为图标按钮补 `aria-label`、增加 `aria-live` 状态区，并为窄屏邮件列表与详情切换补回归测试。
4. **解析性能**：把复杂 DOCX/XLSX/PDF 解析从主线程迁移至 Worker 或 Rust worker，避免大文件操作造成界面卡顿。
## 18. 非 Notion 优化执行记录：密钥回退安全加固（2026-08-01，本地未提交）

### 18.1 本轮目标与边界

本轮继续处理非 Notion 高优先级项 **SEC-03** 中最容易安全落地的部分：禁止在系统凭据库不可用时，为新数据创建可预测的 machine-id 密钥，禁止新建本地 `secret.key` 作为“可靠回退”。

本轮采用保守迁移策略：**不自动删除既有 `secret.key`**。这是因为在未验证 keyring 中的密钥确实可以解密历史文件前，自动删除旧密钥可能造成不可逆的数据恢复失败。旧文件只保留为用户可控的备份/清理对象，待后续增加“验证解密成功后再清理”的迁移流程。

### 18.2 已完成改动

| 优化项 | 实施内容 |
|---|---|
| 应用凭据不再使用可预测回退 | `get_or_create_secret_key` 在 keyring 无记录时生成随机 AES-256 密钥并写入 OS keyring；keyring 读取/写入失败直接返回错误，不再退回 `SHA-256(machine_uid)`。 |
| 历史密钥不再新建本地回退 | 新安装且没有旧 `secret.key` 时，随机历史密钥只写入 OS keyring，不再写入本地 `secret.key`。 |
| 旧历史密钥兼容迁移 | 如果 keyring 没有记录但发现旧 `secret.key`，会校验 32 字节长度并尝试写入 keyring；迁移成功后仍保留旧文件，避免未验证解密能力时误删。 |
| 旧密文兼容读取 | 旧版使用 machine-id 派生密钥加密的设置仍可尝试读取，但该派生逻辑仅保留为 legacy decrypt 路径，不再用于新密钥生成。读取机器标识失败时不再使用固定字符串回退。 |
| 错误可见性提升 | keyring 密钥格式错误、长度错误、访问失败和迁移失败均返回明确错误，不再静默忽略 keyring 写入失败。 |
...
1. 既有 `secret.key` 尚未自动删除，需要后续在“keyring 密钥可以成功解密历史文件”后再提供用户确认清理。
2. `email_history.enc` 仍由前端 `LazyStore` 直接读写，扩展名不能证明内容已加密；统一加密历史存储仍需单独设计迁移格式和前端调用边界。
3. 旧版 machine-id 密文兼容路径仍存在，用于避免升级后已有配置无法读取；待迁移窗口结束后应增加版本标记、重加密和移除 legacy 解密逻辑。
4. 本轮未进行真实 Windows Credential Manager/keyring 可用性联调；需要在用户环境验证首次启动、已有旧密钥迁移、凭据库不可用时的错误提示以及历史恢复路径。

### 18.5 发布前人工核查建议

1. 在隔离环境删除/重命名 keyring 条目但保留测试历史，确认应用不会创建新的本地 `secret.key`。
2. 使用旧版本生成的 `secret.key` 和 `history.enc` 副本，验证迁移后历史可正常解密，并确认旧文件仍未被自动删除。
3. 模拟 keyring 访问失败，确认设置保存失败且不会静默落盘可预测密钥。
4. 确认升级后已有 API Key、Notion Key、邮件密码和 Provider Key 均能读取；若无法读取，界面应要求重新输入，而不是把密文当作明文继续使用。
## 19. 邮件大附件兼容与 AI 分析逻辑评估（2026-08-01）

### 19.1 本轮目标

用户反馈原 IMAP 实现会先读取 `RFC822.SIZE`，再以 5 MB 为阈值直接跳过整封邮件，导致“正文较小但附件较大”的业务邮件无法进入待办分析。

本轮取消了“整封原始 MIME 邮件超过 5 MB 即拒绝下载”的规则，但未改为不受控制地下载整封邮件；改为按照 MIME 结构选择性读取业务相关内容。

### 19.2 下载策略调整（方案 A）

`src-tauri/src/imap_cmds.rs` 现在执行以下流程：

1. 使用 `ENVELOPE BODYSTRUCTURE` 获取主题、发件人、日期和 MIME 部件目录；
2. 只请求非附件的 `text/plain`、`text/html` 和有限数量的内嵌 `image/*` 部件；
3. 跳过普通附件及嵌套 `message/rfc822` 邮件附件，不再因其体积使整封邮件被跳过；
4. 每封邮件最多选择 12 个可读取 MIME 部件；
5. 保留最多 10 张内嵌图片，传输体不超过约 700 KB，解码后不超过 500 KB；
6. 对未返回 `BODYSTRUCTURE` 的非标准服务器，只回退请求 `BODY.PEEK[TEXT]`，不回退到 `BODY.PEEK[]` 整封原始 MIME 下载。

因此，含大附件的邮件可正常下载正文并进入 AI 分析；普通二进制附件不会进入前端历史或模型输入。

### 19.3 本地历史存储边界

取消整封邮件 5 MB 阈值后，为避免历史记录文件随着长正文持续膨胀，新增本地历史预览边界：

| 内容 | 历史保存规则 | AI 分析行为 |
|---|---|---|
| 纯文本正文 | 最多保存前 120,000 字符，并追加可见截断说明 | 仍使用完整选择性正文，由模型输入预算控制最终发送内容 |
| HTML 正文 | 最多保存前 160,000 字符，并追加可见截断说明 | 不改变 HTML 作为纯文本回退的既有处理方式 |
| 内嵌图片 | 保持最多 10 张、单张解码后最多 500 KB | 压缩后作为可选视觉输入发送给模型 |

这不是重新引入“整封邮件 5 MB 下载限制”：限制的是本地历史持久化预览，目的是防止大量长邮件占满应用数据目录。

### 19.4 邮件 AI 分析现状评估

#### 已具备的有效能力

1. **线程降噪**：`parseEmailThread` 优先取最新正文；历史引用链以较低权重提供背景，最多保留约 4,000 字符，降低重复提取过往任务的概率。
2. **多模态输入**：正文、HTML 回退文本和筛选后的内嵌图片可进入同一提取流程；图片会先压缩。
3. **供应商可靠性**：网络错误、超时、408、429 和 5xx 可重试或故障转移；4xx 配置错误不会把相同内容无意义地发送到其他供应商。
4. **结构化输出**：Prompt 要求 JSON，前端验证 `summary`、`todos`、待办 `id` 与 `title`，并保留原始待办快照用于后续人工编辑和反馈。
5. **自动同步顺序**：先完成 AI 提取，再按配置同步 Notion，最后才标记邮件已读；AI/Notion 失败不会提前改变邮件已读状态。

#### 仍需后续优化的逻辑问题

| 编号 | 优先级 | 问题 | 影响与建议 |
|---|---|---|---|
| MAIL-AI-01 | P1 | `tokenLimit` 当前按 JavaScript 字符数截断，而非真实 Token | 中文、英文、HTML 的 Token 比例差异较大。应明确配置名为“最大字符数”，或引入模型 Token 估算和分段摘要策略。 |
| MAIL-AI-02 | P1 | 选择性正文仍会先经 IPC 发送到前端，再由前端截断后调用 AI | 大型纯文本邮件虽不再受附件影响，但仍可能增加 IPC、线程解析和前端内存压力。后续可把分段、摘要和 AI 输入构建下沉到 Rust worker。 |
| MAIL-AI-03 | P1 | AI 返回字段只严格验证了 `id`、`title`、`summary` 和 `todos` | 应继续校验 priority、日期格式、Notion number/checkbox/select/multi-select 枚举，以及待办数量与字段长度，尽早拦截无效模型输出。 |
| MAIL-AI-04 | P2 | 线程切分依赖中英文发件人/转发标识正则 | 特殊模板、HTML 转发和部分国际化邮件可能切分不准。建议增加真实样本脱敏测试集，并在低置信度时展示“包含历史引用”的提示。 |
| MAIL-AI-05 | P2 | 自动偏好学习为 fire-and-forget | 失败只写日志，不会在界面展示或进入重试队列。应改为可追踪的 evolution 状态，并只使用用户明确确认的最终待办作为学习样本。 |
| MAIL-AI-06 | P2 | 邮件历史仍保留压缩后的内嵌图片 | 多封带图片邮件仍可能占用较大本地空间。后续可加入历史图片总量预算、按需加载或只存缩略图。 |
...
|---|---:|
| `cargo fmt -- --check` | 通过 |
| `cargo test` | **9 个用例全部通过**，新增 MIME 部件选择和大于旧 5 MB 阈值文本解码覆盖 |
| `cargo clippy --all-targets -- -D warnings` | 通过 |
| `npx tsc --noEmit` | 通过 |
| `npm test -- --run` | **16 个测试文件、53 个用例全部通过** |

尚未使用真实邮箱执行 IMAP 端到端联调；发布前需使用测试邮箱验证：大附件小正文、纯文本大正文、多层 multipart/alternative、CID 内嵌图片、普通图片附件、无 BODYSTRUCTURE 回退，以及 Outlook/Gmail/企业邮箱的目录和编码兼容性。

## 20. 大模型供应商异常自动轮换核查（2026-08-01）

### 20.1 结论

当前“请求时异常自动轮换”逻辑可正常工作：已启用的服务商会按 priority 从小到大依次调用；当前服务商达到单节点尝试上限后，遇到可重试异常会切换至下一服务商。内置 Provider 走 Tauri HTTP transport，自定义 Provider 走 Rust `request_custom_llm` 代理，二者均接入同一轮换入口。

### 20.2 轮换规则

| 情况 | 当前行为 |
|---|---|
| 网络连接失败、`TypeError`、连接重置等 | 可重试；达到当前服务商尝试上限后轮换 |
| 请求超时 | 可重试；达到当前服务商尝试上限后轮换 |
| HTTP 408、409、425、429、5xx | 可重试；达到当前服务商尝试上限后轮换 |
| HTTP 400、401、403、404 等非 408 的 4xx | 不重试、不轮换，直接返回配置/请求错误 |
| HTTP 2xx 但响应结构缺少有效 `choices[0].message.content` | 不轮换，直接返回响应契约错误 |
| 关闭“异常自动轮换” | 不切换下一个 Provider；当前 Provider 仍按配置执行其自身尝试次数 |
| Provider 未启用或未配置 API Key | 跳过该 Provider |

自定义 Provider Rust 代理产生的超时、网络连接错误会被转换为 `HttpRequestError`，并由同一可重试判定进入轮换流程。

### 20.3 自动化核查覆盖

已在 `src/lib/ai.test.ts` 和 `src/lib/providerTransport.test.ts` 运行 mock 回归测试，覆盖：

1. 网络连接错误后按 priority 轮换到下一个服务商；
2. 429 限流后轮换，且即使配置数组保存顺序颠倒，仍以 priority 顺序调用；
3. 503 服务端异常后轮换；
4. 409 冲突和 425 Too Early 会被当作可恢复 HTTP 异常并轮换；
5. 400 非可重试错误不重试、不轮换；
6. 2xx 但响应不是有效 JSON 时会轮换；有效 JSON 但不符合 OpenAI 响应契约时仍不轮换；
7. 关闭 failover 后，503 不会切换到下一服务商；
8. Rust 代理的 DNS 解析、TLS/证书、连接/读取响应和超时错误会归类为可重试异常。

相关测试：`3` 个测试文件、`20` 个用例全部通过；项目全量前端测试为 `16` 个测试文件、`58` 个用例全部通过。

### 20.4 使用边界

- 该机制是**请求失败后的被动轮换**，不是后台健康检查或自动探测可用性。
- `failoverRetryCount` 当前在代码中表示“单个 Provider 的总尝试次数”，例如设置为 `1` 时会请求该 Provider `1` 次后再轮换；界面中文案“最大重试次数”容易被理解为“首次请求之外的额外重试次数”，建议后续统一为“最大尝试次数”，或调整实现为“首次请求 + 配置的重试次数”。
- 本轮验证使用 mock，不使用真实 API Key，也未对实际第三方服务商制造 429/5xx/网络故障；真实联调仍建议使用两个测试 Provider 验证超时、429、503、400 和备用服务商成功恢复场景。

## 21. 负反馈完善记录：漏提取反馈（2026-08-01，本地未提交）

### 21.1 问题与目标

此前负反馈仅在 `result.todos.length > 0` 时展示，因此 AI 输出空待办数组时，用户无法表达“本应提取出任务但系统漏掉了”的关键负反馈。该场景不应被当作无变化；它属于与“误提取”不同的 **漏提取** 学习样本。

### 21.2 已完成改动

| 能力 | 实施内容 |
|---|---|
| 空结果反馈入口 | 手动提取和邮件历史在待办数组为空时均展示“没有提取到待办？补充告诉 AI”按钮；同步按钮仍只在存在待办时显示。 |
| 两类反馈语义 | 新增 `over_extraction`（误提取）与 `missed_extraction`（漏提取）两类反馈类型，并根据待办数量自动判定。 |
| 持久化与 UI 状态 | `AIResult` 保存 `feedbackType`；处理完成后针对漏提取显示“已记录漏提取反馈”的准确提示，而非将其标为拒绝结果。 |
| 自动演进上下文 | `backgroundReviewAndUpdateFocus` 接收反馈类型；漏提取场景会明确要求模型从用户补充中学习行动信号、责任表达、截止时间和上下文，不能因本次数组为空而忽略。 |
| 邮件状态解耦 | 邮件提交反馈后不再把 `syncedToNotion` 改写为 `true`；关闭的仅是反馈输入框，避免把“已反馈”伪装成“已推送 Notion”。 |
| 回归测试 | 扩展 `feedbackAvailability` 单测，覆盖空结果=漏提取、非空结果=误提取，以及反馈处理中/完成后隐藏入口。 |

### 21.3 验证结果

| 检查项 | 结果 |
|---|---:|
| `npm test` | **16 个测试文件、55 个用例全部通过** |
| `npx tsc --noEmit` | 通过 |
| `npm run lint` | 通过 |
| `npm run build` | 通过 |
| `git diff --check` | 通过 |
...
当提取结果为空时，用户可点击“没有提取到待办？补充告诉 AI”，填写例如“请周五前确认报价应被识别为待办；看到请确认/跟进/提交/截止等表达应提取”。该事件会以 `missed_extraction` 保存并传给自动演进模块。

仍需注意：自动演进当前在静态提示词模式下不会实际调用模型，且其内部失败状态尚未完全回传 UI；这属于后续自动演进状态机完善事项。

### 20.5 可选 401 认证失败备用服务商策略

新增持久化设置 `failoverOnAuthError`，默认值为 `false`，设置页文案为“认证失败（401）时也尝试备用服务商”。

- **默认关闭**：401 继续立即提示当前 Provider 的 API Key/认证配置问题，不向其他 Provider 发送请求。
- **用户主动开启且全局 Failover 开启**：当前 Provider 返回 401 后，不会重复使用同一失效 Key；系统直接按 priority 切换到下一已启用、已配置 Key 的 Provider。
- **全局 Failover 关闭**：即使该选项已勾选，也不会切换 Provider。
- **隐私提示**：界面明确说明，开启后原请求内容会发送给备用服务商。

新增回归覆盖默认 401 不轮换、启用后 401 直接轮换且不进行同 Key 重试，以及设置页勾选后保存持久化。当前项目全量前端测试为 **16 个测试文件、58 个用例全部通过**。

## 22. 历史负反馈回看完善记录（2026-08-01，本地未提交）

### 22.1 问题

负反馈文本此前虽然会写入手动历史和邮件历史的 `explicitFeedback` 字段，但历史 UI 显示不完整：手动历史只显示旧的 `isRejected` 记录，漏提取反馈无法回看；邮件历史没有展示反馈原文的区域。

### 22.2 已完成改动

- 新增共享组件 `FeedbackHistoryCard`，统一显示：反馈类型（误提取/漏提取）、处理状态（学习处理中/已提交）和用户提交的原文。
- 手动历史详情接入该组件；兼容旧 `isRejected` 记录，将其识别为误提取反馈。
- 邮件历史“待办处理”详情在智能分析摘要后接入该组件，可回看邮件场景中的负反馈文字。
- 未填写文字说明时，明确显示“未填写文字说明（仅提交了反馈类型）”，避免用户误以为历史丢失。
- 为反馈类型标签增加无障碍名称，并新增组件测试覆盖：漏提取文本回看、旧误提取记录兼容和无反馈时不渲染。

### 22.3 验证结果

| 检查项 | 结果 |
|---|---:|
| `npm test` | **17 个测试文件、61 个用例全部通过** |
| `npx tsc --noEmit` | 通过 |
| `npm run lint` | 通过 |
| `npm run build` | 通过 |
| `git diff --check` | 通过 |
...
打开手动历史或邮件历史的详情后，只要该记录已提交过显式负反馈，即可看到“已提交的 AI 反馈”卡片。其中包含反馈类别、处理状态和原始反馈文字；漏提取与误提取会以不同标签区分。

## 21. 非轮换异常下的邮件提取重试优化（已完成）

### 21.1 问题与影响

此前邮件扫描器的外层重试循环会对所有异常统一重试。对于 `400`、`403`、`404`、未启用认证故障转移时的 `401`，以及服务商响应契约不兼容等配置类错误，系统会在约 2 秒后用同一服务商再次发起请求；失败邮件还会在后续自动扫描中反复处理，造成无效调用和日志噪音。

### 21.2 本次修复

- 在 `src/lib/http.ts` 增加统一的 `isRetryableRequestError`，仅将网络、超时、`408/409/425/429/5xx` 与显式可恢复错误识别为可重试。
- 邮件扫描器仅对可恢复异常执行 `retryCount + 1` 次尝试；不可恢复异常立即结束本次邮件处理。
- 邮件历史新增 `retryable` 字段，保存失败是否可恢复。
- 自动扫描会跳过已记录为 `retryable: false` 的同一封失败邮件，避免永久配置错误持续消耗调用；用户修复配置后可通过“手动扫描”重新处理。Notion 创建结果未知的场景仍需先人工核对，以避免重复建页。
- Notion 同步结果也透传可重试性：网络/服务端短暂异常可重试，字段映射、权限、参数和不确定创建结果等不自动重试。
- 邮件复核页会提示不可恢复失败需要检查服务商或 Notion 配置，并引导用户手动扫描恢复。

### 21.3 自动化测试

新增回归验证：

1. `400` 非可恢复提取失败不执行邮件内层重复调用；后续自动扫描跳过该邮件；手动扫描允许在配置修复后再次处理。
2. 超时类临时失败会遵守 `retryCount` 重试并在后续成功后结束。
3. HTTP 通用分类同时覆盖 `503` 可重试、`400` 与 `401`（未启用备用策略）不可重试。


## 23. 历史列表负反馈标识完善记录（2026-08-01，本地未提交）

### 23.1 已完成改动

- 在手动历史列表新增紧凑反馈状态标识，且与“已推送”标签可并列显示。
- 在邮件历史列表的审核/推送标签区域新增同一标识，用户无需打开详情即可识别该邮件是否已反馈。
- 标识规则：`⏳ 反馈学习中`、`⚠️ 已反馈：漏提取`、`👎 已反馈：误提取`；旧版 `isRejected` 记录兼容显示为误提取。
- 使用共享 `FeedbackStatusBadge` 组件，保证两类历史列表与详情中的反馈语义一致，并为标签提供无障碍名称。
- 扩展组件测试，覆盖漏提取、处理中和旧误提取记录三种列表状态。

### 23.2 验证结果

| 检查项 | 结果 |
|---|---:|
| `npm test` | **17 个测试文件、66 个用例全部通过** |
| `npx tsc --noEmit` | 通过 |
| `npm run lint` | 通过 |
| `npm run build` | 通过 |
| `git diff --check` | 通过 |
...
现在在历史列表中就能直接判断：该记录是否已经提交过负反馈、属于漏提取还是误提取、反馈是否仍在学习处理中；打开详情后可继续查看完整反馈原文。

## 22. 重启后加密凭据失效修复（2026-08-01，本地未提交）

### 22.1 根因

近期安全存储改造将应用设置的 AES 密钥改为仅依赖操作系统凭据库。部分 Windows Credential Manager 环境会在当前进程内允许写入，但无法在下次启动时读取相同条目；解密路径又会在找不到条目时生成新的随机密钥。结果是上次保存的密文无法解密，前端收到空字符串，表现为 API Key、Notion Key 和邮箱密码在每次重启后都需要重新输入。

### 22.2 修复

- 仍优先使用 Windows Credential Manager 中的有效随机 AES 密钥。
- 新增应用数据目录中的 **DPAPI 保护恢复副本**：恢复副本由当前 Windows 用户的 DPAPI 加密，不能由其他用户或仅复制文件的攻击者解密。
- 凭据库读取失败时从 DPAPI 副本恢复同一密钥，并尽力回写凭据库；不再生成一次性随机密钥。
- 新建密钥时必须先成功写入 DPAPI 保护副本；若保护副本无法写入则保存操作失败，避免产生下次启动无法解密的密文。
- 保留旧设备绑定密钥的解密兼容分支，便于读取早期版本保存的设置。

### 22.3 兼容性说明

已经使用“仅凭据库随机密钥”保存、且对应 Windows 凭据库条目已丢失的密文在密码学上无法恢复；此类凭据需最后重新输入一次。修复后新保存的凭据同时受 Credential Manager 与 DPAPI 恢复路径保护，可跨应用重启使用。

### 22.4 自动化验证

新增 Rust 单元测试，覆盖凭据库密钥格式验证，并在 Windows 上实际验证 DPAPI 加密/解密回环。

## 23. 大模型供应商超时时间调整（2026-08-01，本地未提交）

- 单个大模型供应商请求超时时间由 30 秒调整为 **180 秒（3 分钟）**。
- 内置供应商通过前端 HTTP 通道使用 `LLM_REQUEST_TIMEOUT_MS`。
- 自定义供应商 Rust 代理的总请求超时时间同步调整为 180 秒。
- 自定义供应商建连超时调整为 60 秒；连接建立后最长等待响应 180 秒。
- 普通 HTTP 请求（例如 Notion）仍保持原来的 30 秒默认超时，不受本次调整影响。

## 24. 大模型请求发送失败与待办输出兼容性修复（2026-08-01，本地未提交）

### 24.1 日志核查结论

实测日志中的三封邮件暴露出两类独立问题：

- UID 1006 在约 153 秒后收到 Tauri HTTP 的 `error sending request for url (...)`。该历史记录发生在此前 180 秒应用层超时之前，是底层请求通道返回的传输错误，不是应用主动超时。
- UID 1008 与 UID 357 已成功收到模型回复，但解析时因第一个待办缺少 `id` 或 `title` 失败。尤其是在启用 Notion 动态字段映射时，旧提示词只输出映射字段，未始终要求解析器依赖的字面量 `title` 字段，容易造成模型输出契约不一致。

### 24.2 已完成修复

- 扩充 HTTP 可恢复传输错误分类：`error sending request`、`failed to send request`、`error while sending` 等 Tauri/reqwest 常见表述现在会被识别为可重试错误。
- 上述错误会进入现有 Provider 重试、按优先级故障转移，以及邮件扫描器的 `retryCount` 外层重试逻辑；因此 AI 中台请求发送失败时可顺位轮换到备用供应商。
- 重建待办 JSON 示例：无论是否启用动态 Notion 字段，均始终包含 `id`、`title`、`priority` 与 `planned_date`。提示词同时明确禁止用 `task`、`content`、`name` 或 Notion 字段名替代 `title`。
- 增加兼容规范化：兼容 `task`、`content`、`name`、`text`、`description` 以及已启用的 Notion 标题字段作为标题来源；服务商遗漏 ID 时生成稳定的本地 ID，保留模型原始字段供 Notion 映射同步。
- 增加安全诊断日志：仅记录顶层字段和单条待办字段名、采用的兼容来源，不记录邮件正文或完整模型响应。

### 24.3 自动化回归覆盖

新增并通过以下场景：

1. `error sending request for url` 被识别为可恢复传输错误。
2. 内置 AI 中台发生该错误后，自动轮换至下一内置 Provider。
3. 邮件扫描器遇到该错误时，依据 `retryCount` 执行邮件级重试。
4. 启用 Notion 标题字段后，提示词仍包含固定 `title`，并可将供应商返回的 Notion 标题字段规范化为系统待办标题，同时生成稳定 ID。

验证结果：`npm test` 为 **17 个测试文件、73 个用例全部通过**；`npx tsc --noEmit`、`npm run lint`、`npm run build` 与 `git diff --check` 均通过。

### 24.4 残余风险与建议

- 若服务商返回的待办既没有常见标题字段，也没有已启用的 Notion 标题字段，系统会明确失败并记录字段名，避免凭空生成错误待办。
- 格式不兼容目前优先通过提示词与本地兼容层处理，不会自动将同一邮件发送给其他供应商；这可避免无意义的跨供应商数据发送和额外成本。若后续仍频繁发生，可再评估增加一次独立的格式修正调用。

## 25. Notion 同步后正反馈学习优化（2026-08-04，本地未提交）

### 25.1 已完成改动

- 将“本次待同步集合”和“用户认可学习集合”拆分：Notion 请求仍只发送未同步待办，AI 正反馈则使用当前所有勾选待办。
- 在同步前创建不可变学习快照，过滤运行时 `selected`、`synced` 字段，避免部分同步重试时丢失已成功同步的待办。
- 增加稳定正反馈指纹，防止同一批用户选择因 Notion 重试或页面重渲染被重复提交给大模型。
- 增加正反馈状态：`processing`、`pending_verification`、`completed`、`unchanged`、`skipped`、`failed`。
- Notion 存在 `needsVerification` 未知结果时，暂缓 AI 学习；用户确认页面状态后再启动正反馈分析。
- 主页面和邮件历史页面统一显示正反馈处理状态，并支持 AI 学习失败后重试。
- 自动演进结果细分为：规则已更新、正反馈已记录但规则无需变化、跳过和失败；完成或失败都会向主窗口发送通知事件。

### 25.2 关键行为

- 用户未修改任何待办并完整同步时，不启动正反馈学习。
- 用户取消、修改或新增待办后同步，只要至少有一条 Notion 同步成功，就使用全部当前勾选待办进行学习。
- 部分同步成功时，已成功同步的待办仍保留在正反馈样本中。
- 同一正反馈指纹在 `processing`、`completed` 或 `unchanged` 状态下不会重复调用 AI；`failed` 状态允许重试。

### 25.3 自动化验证

新增覆盖：

1. 已同步待办不会从正反馈学习样本中消失。
2. 取消勾选待办会被识别为正反馈变化，并生成稳定指纹。
3. 相同指纹在处理中或已完成时不会重复学习，失败状态可以重试。
4. 用户选择发生变化但 AI 判断无需更新规则时，会显示“正反馈已记录”。
5. 用户未修改结果时跳过模型调用。
