...
- UID 1006 在约 153 秒后收到 Tauri HTTP 的 `error sending request for url (...)`。该错误发生在 180 秒应用层超时之前，是底层请求通道返回的传输错误，不是应用主动超时。
