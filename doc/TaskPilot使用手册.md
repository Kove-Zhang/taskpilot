...
### 1.2 沉浸式桌面交互、隐身机制与全局窗口化模式
TaskPilot 提供两种截然不同的运行形态，以适应您在不同场景下的办公需求。

#### 形态一：Spotlight 聚光灯模式（默认）
...
#### 形态二：全局常驻窗口化模式 (Global Window Mode)
如果您希望将其作为一块常驻桌面的大型工作面板（尤其在处理大量双源历史数据或撰写超长提示词时）：
* **开启方式**：点击右上角 `⚙️ 设置`，在“系统与日志”标签页中勾选“开启窗口化模式”。
* **双模态无缝切换**：此时应用窗口会瞬间扩展为大宽屏布局，并**永久解除失去焦点即隐藏的限制**，变成一个可以和普通应用程序一样停靠、最大化/最小化的常驻窗口看板。
...
* **文本截断保护阀与安全阈值 (Token Limit)**：
  为防止您在处理批量文档时不小心粘贴数万字日志代码、无底洞冗余数据或乱码，导致接口抛出 Context Length Exceeded (400) 异常甚至触发超额费用飙升，系统在 AI 设置页提供了可选的 `Token Limit` 门闸（默认值 `8000`）。
  * **日常高频邮件与微信对话**：推荐保持默认 `8000`，对长文本实施最稳妥的安全防护。超出部分将自动安全修剪并提示：`...(文本已截断以保护 Token 限制)`。
  * **全量白皮书 / 大批量周报解析**：当您在处理多篇技术规约或复杂图文汇总，且确定预算充足时，可前往设置将其放宽至 `16000` 或更高，以确保全量上下文无损送入大模型。
...
### 2.3 个人关注焦点、AI 提示词智能润色与自动持续演化 (Prompt Focus)
...
> [!IMPORTANT]
> **🚀 黑科技：深度分析历史记录以生成偏好 (自动持续演化)**
> 我们深知用户的“工作重心”会随着项目推进而不断迁移。相比手动去改写关注点，TaskPilot 支持“逆向行为推演”！
> 当您点击 **[ 深度分析历史记录以生成偏好 ]** 按钮时：
> 1. 系统底层会静默抓取两大独立历史日志：您在主界面手动提取过的历史任务，以及后台邮件监听自动推送至 Notion 成功的历史待办。
> 2. 将双源数据按时间线降序无缝融合，截取最近发生的 30 次高频高价值工作。
> 3. 剥离敏感凭据后发送给大模型，让 AI 根据您实际的工作轨迹，“倒推”出您最近最关心的焦点。
> 这是一套真正能够“越用越懂你”的自动演进引擎。
* **⛶ 全屏沉浸式放大编辑与保存 (Zen Expand Editor)**：
  面对数千字的业务规范或超长指令宏，卡片内 ~120px 的狭窄多行输入框极易带来滚动排版的局促感。现在，您只需点击右上角新增的 **`[ ⛶ 放大编辑 ]`** 按钮：
  1. **黄金占比大视野**：系统在当前可视窗口上方悬浮呼出 900x640 毛玻璃高定沉浸式工作台（约占 1024×768 视窗的 88% 黄金区域），配合 `leading-relaxed` 1.75 倍舒适行高，长篇规则一览无余！
  2. **实时数据监控**：顶栏右侧集成 **实时字数与行数统计 Badge** (`字数: XXX | 行数: XX`)，让您对提示词规模了如指掌。
  3. **原位润色与对比撤销**：大窗底部直接内含 `[ ✨ AI 智能润色 ]` 与 `[ ↩️ 撤销润色 ]`，在大视窗下即可按击调用 AI 对现有规则进行扩展优化并支持即时对比与回滚。
  4. **人体工学快捷键与防丢失安检 (`Dirty Check`)**：支持 Windows 下按下 **`Ctrl + Enter`** 或 macOS 下按下 **`Cmd + Enter`** 极速保存写回并退出；如果在大窗中输入和修改了大量规则文字却忘按保存直接按 `Esc` 或点击遮罩关闭，系统将立刻触发防误触拦截确认弹窗（`"检测到未保存的修改，是否确认放弃？"`），彻底为您杜绝心血文字误触丢失的痛点！
...
* **单属性专属微提示词 (Prompt Hint) 深度控制**：在每个勾选属性后方，系统内置了一个文字框。当面临复杂的 Notion 下拉选框 (`Select`/`Multi-select`)、人员指派或日期推导时，普通的全局提示词往往不够精准。您只需在此输入针对该单列的微约束：
  * **枚举对齐案例**：针对属性 `[优先级]` (白名单包含 High, Medium, Low)，填入 Hint：`"如果文字中包含服务器宕机、客户投诉、逾期等高危词，必须严格填写 High，日常常规优化填写 Low"`。AI 会在处理这单列时严格参照该 Hint 选取合法枚举！
  * **日期格式案例**：针对属性 `[截止日期]`，填入 Hint：`"必须根据发信时间推算出确切的 YYYY-MM-DD 格式，若无明确时间则留空不要瞎编"`。
...
### 4.2 监听目标文件夹支持“多收件箱/子目录并发轮询” (Target Folders)
默认情况下系统监听 `INBOX`（主收件箱），但 TaskPilot 底层实现了精妙的**多文件夹并发轮询引擎**：
* **配置实操**：如果您将不同客户或项目的信件在邮箱内自动归档到了不同的子文件夹，您可以在“监听文件夹”配置框中填入多个以英文逗号分隔的目录名称（例如 `INBOX, Work, VIP客户, Urgent`）。
* **自动化效果**：每次触发扫描时，系统后台会自动顺次遍历所有填入的文件夹，一次性将多个收件箱或子分类中的待办一网打尽！

### 4.3 智能双轨扫描时间视界：后台定时 (autoReadDays) vs 手动初始化 (manualReadDays)
为了在“极致节省带宽开销”与“严防假假期漏单”之间达成绝佳平衡，系统对邮件抓取的时间跨度采取了**双轨独立时间视界**：
* **后台定时静默扫信 (`autoReadDays`，默认 3 天)**：当定时任务自动运行（如每小时或早上9点冲刺）时，系统仅仅拉取最近 3 天内的新邮件。这样既轻量又高速，能在毫秒级完成静默比对；
* **人工手动拉取 / 首次挂载初始化 (`manualReadDays`，默认 7 天)**：当您人工点击“立刻拉取”按钮，或者第一次配置绑定该邮箱时，底层会自动扩大搜索视界至最近 7 天！确保周末、三天假期甚至跨周遗漏的重大工单被一次性全量找回。

### 4.4 网络抗抖动与局部自愈重试 (Retry Count)
在企业复杂的办公网络或 VPN 环境下，偶发的网络抖动、DNS 瞬时丢包可能导致定时邮件扫描中断。
* **局部自愈机制**：在设置选项中您可以配置 `重试次数 (Retry Count)`（默认 1 次）。一旦 IMAP 握手超时或网络中断，后台监护进程会自动抓取异常并进行局部重试，无需人工干预即可在网络恢复后自动接续执行。
* **🛡️ 防重试风暴与日志原地去重黑魔法 (`Deduplication Engine`)**：
  很多传统监控工具一旦遇到网络连接失败或 AI 处理超时触发重试时，会在日志列表中无限插入相同的报错记录，导致控制台被“红色报错风暴”彻底刷屏阻塞。
  TaskPilot 创新的底舱调度器采用了基于复合主键 **`[目标文件夹 + 邮件 UID]`** 的原地映射比对算法：如果某封邮件处理异常并进入重试或重新拉取流程，系统会精确定位历史列表中**已存在的旧日志卡片**，仅在原位直接覆写更新最新时间戳和报错摘要，绝对杜绝产生重复刷屏的红色日志块，让您的工作台永远保持高信噪比与清爽整洁！
...
### 5.3 双模监控视图与逐条深度审核模式 (Review Mode)
除了默认的卡片聚合列表视图（方便您快速扫描全天抓取的所有邮件与待办进度），我们在邮箱监控工作台顶端提供了 **`[ 📋 全景历史聚合 | 🔍 逐条深度审核 ]`** 的双重视图模式切换：
* **🔍 逐条深度审核模式 (Interactive Review Mode)**：
  如果您负责审批重大商务合同或者复杂的运维工单，您可以在该模式下像翻阅电子公文一样，按时间轴逐一专注核对单封邮件：
  1. **原件深度核对**：直接展示发件人的 **原生 HTML 白底排版或深色排版**，并完整解析邮件附带的 **内联图片 (`data:image`) 与附件清单**，做到审核与原件完全不脱钩；
  2. **全链路人工干预与对齐 (Manual Todo CRUD)**：您绝不需要完全依赖和被动接受 AI 抓取的数据！对于 AI 提炼出的待办列表，您可以在页面上手动 **`[ ✏️ 编辑 ]` 待办标题与优先级**、**`[ 🗑️ 删除 ]` 误抓或不重要的条目**，甚至支持直接点按 **`[ ➕ 手动新增待办 ]`** 把漏掉的细节手动录入；
  3. **确认已读联动**：完成核对后，点击 **`[ ✅ 审核确认并标记已读 ]`**，系统将在本地更新对准后的 JSON 快照，同时向邮箱服务器发送 `\Seen` 指令将远端邮件同步标记为已读！

### 5.4 Notion 闭环同步空选与已同步安全防线 (Empty Sync Guard)
在主工作台或邮箱历史卡片中点击 `[ 🚀 同步至 Notion ]` 时，系统底层内置了极为人性化的**安全防线拦截断言**：
* **自动过滤与全量把控**：系统底层会在同步前自动筛查并过滤掉所有您取消勾选的条目，以及此前已经被标记为 `synced: true` 的已同步条目。
* **空选与误操作人性化提醒**：如果您在点击同步按钮前，不小心取消了所有任务的勾选，或者您点开的是一张原本就**已经全量同步完成**的历史旧卡片，系统绝不无声静默，也不会把空数组扔给后端，而是立即跳出友善的警示窗提示：
  `"当前没有可同步的待办事项：您选中的条目可能已全部同步至 Notion，或未勾选任何有效事项。"`
  从而让每一位用户在进行闭环推流时，对系统流转状态心中有数、不存疑虑！
...
| **在放大编辑大窗修改内容后，按 Esc 或点击外部关闭时为何弹出警示框？** | 触发了系统的**未保存变更防触机制 (`Dirty Check`)**。当检测到当前编辑器内文本已发生修改但尚未触发写回时，系统会主动拦截挂起，防止心血规则误删。 | 如确认需放弃本次修改，点击弹窗中的“确认”即可关闭；如想保存，建议习惯使用快捷键 `Ctrl + Enter` 或 `Cmd + Enter` 一键极速保存写回。 |
| **点击“同步至 Notion”时提示“当前没有可同步的待办事项”？** | 触发了系统的**空选安全防线 (Empty Sync Guard)**。您选中的当前待办子列表中，可能所有勾选任务均已被标记为 `synced: true`，或您手动取消了所有待办的复选框勾选。 | 检查卡片列表中待办事项的复选框勾选状态，确认是否存在未同步的具体新任务。 |
| **网络故障或接口超时触发重试扫信时，为什么没有产生一堆重复报错日志？** | 触发了底仓**防重试风暴去重引擎 (`Deduplication Engine`)**。系统采用 `[文件夹 + UID]` 复合主键映射，发生重试时会在内存中精准原位修改历史卡片的状态和报错信息，绝不盲目追加。 | 您可放心保持高频监听与自愈重试，无需手动清理刷屏报错日志，系统将始终自动为您维护整洁高信噪比的工作台。 |
